...
def build(p):
    ...
    for i in range(x_s+1):
        for j in range(y_s+1):
            # If the address is outside the maze definion
...
def victory():
    metric_full = cur.execute(str("SELECT time,direction,p_x,p_y,proximity FROM MazeGameMetrics WHERE time > "+str(t_start))).fetchall()
    metric_posits = cur.execute(str("SELECT p_x,p_y FROM MazeGameMetrics WHERE time > "+str(t_start))).fetchall()
    for r in metric_full:
        print(r)
    mf.destroy()
    Can = Canvas(root,background='white')
    Can.grid(row=0,column=0)
    root.title("   Y o u   W i n !   ")
    clr = 'white'
    for i in range(x):
        for j in range(x):
            if (i,j) == goal: clr = 'blue'
            elif (i,j) in metric_posits: clr = 'green'
            else: clr = 'white'
            Frame(Can,background=clr,height=5,width=5).grid(column=i,row=j)
    root.update()
    root.mainloop()
    con.close()
    exit()
...
mf = Canvas(root,background='white')
